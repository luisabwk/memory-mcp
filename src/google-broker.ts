...
export class GoogleBroker {
  ...
  private pending = new Map<string, StoredPending>();
  ...
  constructor() {
    ...
    this.allowed = new Set(
      allowed.split(',').map((e) => e.trim().toLowerCase()).filter(Boolean),
    );
  }
  ...
  async verifyCallback(state: string | undefined, code: string | undefined): Promise<PendingAuth> {
    ...
    if (!stored) throw new BrokerError(400, 'Unknown or expired login session');
    ...
    if (!idToken) throw new BrokerError(502, 'No id_token from Google');

    let payload: { email?: string | null; email_verified?: boolean | null } | null | undefined;
    try {
