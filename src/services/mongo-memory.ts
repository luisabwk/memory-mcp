...
/** Mongo's on-disk shape: same fields as Memory, but timestamps are Date and _id is the UUID. */
interface MemoryDoc {
  _id: string;
  content: string;
  embedding?: number[];
...
function toMemory(doc: MemoryDoc): Memory {
  return {
    ...
    sector: doc.sector as Memory['sector'],
    source_type: doc.source_type as Memory['source_type'],
...
export class MongoMemoryService {
  ...
  private col() {
    return this.db.collection<MemoryDoc>(COLLECTION);
  }

  async storeMemory(memory: MemoryInsert): Promise<Memory> {
    if (!memory.user_id) throw new Error('storeMemory: user_id is required');
  ...
  async queryMemories(params: MemoryQueryParams, queryEmbedding: number[]): Promise<MemoryQueryResult[]> {
    ...
    if (!user_id) throw new Error('queryMemories: user_id is required');

    const filterClauses: Document[] = [{ user_id: { $eq: user_id } }];
    if (sector) filterClauses.push({ sector: { $eq: sector } });
    if (source_type) filterClauses.push({ source_type: { $eq: source_type } });
    if (source_path) filterClauses.push({ source_path: { $eq: source_path } });
    if (project_name) filterClauses.push({ project_name: { $eq: project_name } });
    if (tags && tags.length > 0) filterClauses.push({ tags: { $all: tags } });
    ...
    const docs = await this.col().aggregate<MemoryDoc & { similarity: number }>(pipeline).toArray();
  ...
  async listMemories(params: MemoryListParams): Promise<{ memories: Memory[]; total: number }> {
    ...
    if (!user_id) throw new Error('listMemories: user_id is required');

    const filter: Filter<MemoryDoc> = { user_id };
    if (sector) filter.sector = sector;
    if (source_type) filter.source_type = source_type;
    if (source_path) filter.source_path = source_path;
    if (project_name) filter.project_name = project_name;
  ...
  async getMemory(id: string, user_id: string): Promise<Memory> {
    if (!user_id) throw new Error('getMemory: user_id is required');
    const doc = await this.col().findOne({ _id: id, user_id });
    if (!doc) throw new Error(`Failed to get memory: not found`);
  ...
  async reinforceMemory(id: string, user_id: string): Promise<Memory> {
    if (!user_id) throw new Error('reinforceMemory: user_id is required');
    await this.updateAccessTracking(id, user_id);
    const doc = await this.col().findOne({ _id: id, user_id });
    if (!doc) throw new Error(`Failed to reinforce memory: not found`);
    return toMemory(doc);
  }

  private async updateAccessTracking(id: string, user_id: string): Promise<void> {
    await this.col().updateOne(
      { _id: id, user_id },
      { $set: { last_accessed_at: new Date() }, $inc: { access_count: 1 } },
    );
  }
}
