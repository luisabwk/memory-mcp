...
export async function createMemoryMcpServer(): Promise<Server> {
  assertEnv();
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
      switch (name) {
        case 'memory_store':
          return { content: [{ type: 'text', text: JSON.stringify(await storeTool.execute(args as unknown as Parameters<MemoryStoreTool['execute']>[0]), null, 2) }] };
        case 'memory_query':
          return { content: [{ type: 'text', text: JSON.stringify(await queryTool.execute(args as unknown as Parameters<MemoryQueryTool['execute']>[0]), null, 2) }] };
        case 'memory_list':
          return { content: [{ type: 'text', text: JSON.stringify(await listTool.execute(args as unknown as Parameters<MemoryListTool['execute']>[0]), null, 2) }] };
        case 'memory_get':
          return { content: [{ type: 'text', text: JSON.stringify(await getTool.execute(args as unknown as Parameters<MemoryGetTool['execute']>[0]), null, 2) }] };
        case 'memory_reinforce':
          return { content: [{ type: 'text', text: JSON.stringify(await reinforceTool.execute(args as unknown as Parameters<MemoryReinforceTool['execute']>[0]), null, 2) }] };
        default:
